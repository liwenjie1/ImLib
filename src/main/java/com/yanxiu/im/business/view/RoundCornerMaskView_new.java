...
public class RoundCornerMaskView_new extends View {
    ...
    public RoundCornerMaskView_new(Context context) {
        super(context);
        setup(context,null);
    }

    public RoundCornerMaskView_new(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        setup(context,attrs);
    }

    public RoundCornerMaskView_new(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void setup(Context context, @Nullable AttributeSet attrs) {
        mColor = Color.RED;
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.RoundCornerMaskView_new);
            mColor = ta.getColor(R.styleable.RoundCornerMaskView_new_mask_color, 0xffffff); //默认白色
            mRadius = ta.getDimensionPixelSize(R.styleable.RoundCornerMaskView_new_corner_radius, 10);
        }
    }
