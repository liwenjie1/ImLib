...
public class ZoomImageView extends ImageView {
    ...
    public Matrix getOuterMatrix(Matrix matrix) {
        ...
        Log.i(TAG, "getOuterMatrix: " + matrix);
        return matrix;
    }
     ...
     * Matrix.ScaleToFit.FILL =  fitxy
     * Matrix.ScaleToFit.CENTER =  inside
     * Matrix.ScaleToFit.START =  inside
     *
     * @param matrix 用于填充结果的对象
     * @return 如果传了matrix参数则将matrix填充后返回, 否则new一个填充返回
     */
    public Matrix getInnerMatrix(Matrix matrix) {
        ...
        if (isReady()) {
            ...
            //计算矩阵
            switch (findBestOriginMatrix(tempSrc,tempDst)){
                case 0:{
                    //应用 宽图  采用inside 方式显示
                    matrix.setRectToRect(tempSrc, tempDst, Matrix.ScaleToFit.CENTER);
                    break;}
                case 1:{
                    //计算 长图 采用 宽度充满 并以顶部对齐的方式缩放  能否缩小到 center的形式？
     ...
     * 根据图片的尺寸 选择适合的初始显示缩放
     * */
    private int findBestOriginMatrix(RectF sourceRectF,RectF viewRectF){
        //宽图 采用center
        float srcW=sourceRectF.width();
        float srcH=sourceRectF.height();
        float dstW=viewRectF.width();
        float dstH=viewRectF.height();
     ...
     * 宽度撑满
     */
    private void transformMatrix(Matrix matrix) {
        if (matrix == null) {
            matrix=new Matrix();
        }
        matrix.reset();
    ...
    private float getSmallestScale(){
       ...
       float srcW=tempSrc.width();
       float srcH=tempSrc.height();

       float dstW=tempDst.width();
       float dstH=tempDst.height();

       if (srcW/srcH > dstW/dstH){
           //正常的fitcenter情况
           return 1f;
       }else {
           //长图情况 计算 innerMatrix 缩放到 fitcenter 的缩放比例
           //1 获取fitcenter矩阵
           Matrix smallMatrix=new Matrix();
           smallMatrix.setRectToRect(tempSrc, tempDst, Matrix.ScaleToFit.CENTER);
           Matrix innerMatrix=getInnerMatrix(MathUtils.matrixTake());
           //获取两个矩阵的 缩放比例值
           float[] minScale=MathUtils.caculateScaleOfTwoMatrix(innerMatrix,smallMatrix);
           //获取 高度比例
           float minHScale=minScale[1];
    ...
    private void scaleEnd() {
        ...
        /**
         *朱晓龙 on 2018/6/7 11:29. 由于 前面修改了不同图片的innerMatrix 这里的1 不能使用 需要根据不同的图进行不同的计算
         * 目前只有两种情况  1、采用fitcenter 的图片  那么 数值依然是1
         *                  2、采用宽度填充的图片 数值需要转换为 fitcenter情况的值  （长图的最小情况为fitcenter）
         */
        float smallestScale=getSmallestScale();
        //如果当前 的缩放比例 比 fitcenter 时还小 需要修正 修正值为恢复到fitcenter 的比例 smallestScale:outerScale
        if (outerScale * scalePost < smallestScale) {
            scalePost = (smallestScale / outerScale);
        }
        //如果缩放修正不为1，说明进行了修正
        if (scalePost != 1f) {
            change = true;
    ...
    public static class MathUtils {
        ...
        /**
         * 计算两个矩阵的缩放倍数  matrix1 * a =  matrix2
         * */
        public static float[] caculateScaleOfTwoMatrix(Matrix matrix1, Matrix matrix2){

            if (matrix1 == null|| matrix2==null) {
                return new float[]{1f,1f};
            }
            //矩阵1 的 值
            float[] matrixValue1=new float[9];
            //矩阵2的值
            float[] matrixValue2=new float[9];

            matrix1.getValues(matrixValue1);
            matrix2.getValues(matrixValue2);

            float sx1= matrixValue1[0];
            float sx2= matrixValue2[0];

            float sy1= matrixValue1[4];
            float sy2= matrixValue2[4];

            if (sx1==0||sy1==0) {
                return new float[]{1f,1f};
            }
            return new float[]{sx2/sx1,sy2/sy1};
        }
